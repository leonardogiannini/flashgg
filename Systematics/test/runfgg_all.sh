...
outdir="checkTH_ws_"${year}"_"${date}"_"${tag}
...
if [ "$tag" == "data" ]; then
    ...
    fggRunJobs.py --load wsJSONs/legacy_runII_v1_${year}_${tag}.json -d ${outdir} workspaceStd.py -n 300 --no-copy-proxy -b htcondor --stage-to "gsiftp://gftp.t2.ucsd.edu"${stageDir} -q workday doHTXS=True tthTagsOnly=True doSystematics=False doL1Prefiring=False
